...
if ~exist('syll_max','var')
    syll_max=compute_ave_1pimaging(phan,model_starts,...
...
% get the features, try logistic regression, can we differentiate these two
% populations at all??
% TODO: combine all of the "simulated" population data to compare w/
% photometry
...
for i=1:length(phan.stats.corr_scalars)
    ...
    for j=1:length(use_scalars)
        ...
        %tmp_pop(:,end+1)=corr(phan.stats.corr_scalars(i).(use_scalars{j}),mean(phanalysis.nanzscore(phan.stats.corr_imaging(i).data),2),'rows','pairwise','type','pearson');
    end
...
%     tmp_std=cellfun(@(x) nanstd(zscore(x),[],3), {model_starts.imaging(:,i).wins},'uniformoutput',false);
%      tmp_mu=cellfun(@(x) nanmean(zscore(x),3), {model_starts.imaging(:,i).wins},'uniformoutput',false);
%     tmp_std=cellfun(@(x) nanmean(x(ave_vec,:)),tmp_std,'uniformoutput',false);
%      tmp_mu=cellfun(@(x) nanmean(x(ave_vec,:)),tmp_mu,'uniformoutput',false);
%   
    %tmp_mu=nanmean(phan.stats.corr_imaging(i).data);
...
save('~/Desktop/phanalysis_images/cell_prediction_performance_twocolor_with_rnd.mat','perf');
%%
