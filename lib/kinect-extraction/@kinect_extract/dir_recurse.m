function FILES = dir_recurse(DIR, FILTER, MAXDEPTH, MAXDATE, TAG_NAME, TAG_FILE, FILES, DEPTH, SKIP)
...
for i = 1:length(raw_listing)
    ...
    if raw_listing(i).name(1) ~= '.' & raw_listing(i).isdir & ((daysdif(raw_listing(i).datenum, now)) < MAXDATE)
        FILES = kinect_extract.dir_recurse(fullfile(DIR, raw_listing(i).name), FILTER, MAXDEPTH, MAXDATE, TAG_NAME, TAG_FILE, FILES, DEPTH, SKIP);
    end
