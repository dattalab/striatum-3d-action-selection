#!/bin/bash
#
# Finds all data extractions in a given directory, then computes PCs and scalars, saves to _analysis
...
#   -i|--input (string): directory with extractions (will search recursively) (default: pwd)
...
#   -q|--queue (string): cluster queue/partition to submit to (default: priority)
#   -t|--wall-time (string): job wall time (hh:mm:ss, default: 48:00:00)
#   -n|--workers (int): number of workers (default: 20)
#   -m|--mem-usage (int): RAM usage in MB (default: 100000)
...
#
# Example:
# $ kinect_extract_get_projections.sh -i ~/dir_with_lots_of_extractions/ -c
#
#######################################
...
SUBMIT=true
SUBMIT_QUEUE=priority
SUBMIT_WALL_TIME='48:00:00'
SUBMIT_WORKERS=20
SUBMIT_MEM_USAGE=100000
MATLAB_PATH=""
DRY_RUN=false
...
if [ $# -eq 0 ]; then
  ...
  echo "";
  echo "[input] = Input directory to process (directory with previously extracted data)";
  echo "Arguments:";
  echo "  -c|--has-cable (flag): does the data have a cable? (default: True)"
  echo "  -i|--input (string): input directory to process"
...
if [[ -e $INPUT ]]; then
  ...
  if [[ $ENVIRONMENT = o2 ]]; then
    CMD="${MATLAB_PATH}matlab -nodisplay -r \\\"kinect_extract_get_all_projections('$INPUT',$MATLAB_WORKERS,'$BATCH_PROCESSOR');exit;\\\""
  else
    CMD="${MATLAB_PATH}matlab -nodisplay -r \"kinect_extract_get_all_projections('$INPUT',$MATLAB_WORKERS,'$BATCH_PROCESSOR');exit;\""
  fi
